...
mongoose.connect(process.env.MONGOLAB_URI || 'mongodb://test:test@ds031832.mlab.com:31832/heroku_mk2bx4cj', function (error) {
...
var RestaurantDiscountSchema = require('./models/restaurantdiscount');
...
var RestaurantDiscount = mongoose.model('RestaurantDiscount', RestaurantDiscountSchema);
...
lunchedin.updateRating = function (rating) {
    console.log('lunchedin.updaterating');
    var rid = rating.id;
    delete rating["id"];
        Restaurant.findOne({
            "_id": ObjectId(rid)
        }, function (err, getrestaurant) {
            if (err || getrestaurant == null) console.log("Error(1440):", err);
            else {
                var updrestaurant = getrestaurant;
                var allrating = [];
                if (typeof updrestaurant.rating != 'undefined')
                    allrating = updrestaurant.rating;
                allrating.push(rating);
                updrestaurant.rating = allrating;
                updrestaurant.save();
            }
        });
...
*
*/
lunchedin.addCompany = function (company) {
   // console.log('lunchedin.addCompany');
   // console.log(company);
    if (company.name != undefined) {
      
        addToDatabase(Company, company, "Company", null);
      }

};
lunchedin.updateCompany = function (company) {
    console.log('lunchedin.addCompany');
    console.log(company);
    var cid = company.id;
    console.log(cid)
    ...
    if (company.name != undefined) {
        Company.findOne({
            "_id": ObjectId(cid)
        }, function (err, getcompany) {
            if (err || getcompany == null) console.log("Error(1440):", err);
            else
            {
                var updcompany = getcompany;
                updcompany.name=company.name;
                updcompany.country = company.country;
                updcompany.city = company.city;
                updcompany.branchname = company.branchname;
                updcompany.regdate = company.regdate;
                updcompany.userunitprice = company.userunitprice;
                updcompany.save();
            }
        });
    }
...
lunchedin.addDiscount = function (discount) {

    addToDatabase(RestaurantDiscount, discount, "RestaurantDiscount", null);
};
...
lunchedin.updateUser = function (usr) {
    console.log('lunchedin.updateuser');
    console.log(usr);
    var uid = usr.id;
    console.log(uid)
    delete usr["id"];
    ...
    if (usr.name != undefined) {
        User.findOne({
            ...
            if (err || getuser == null) console.log("Error(1440):", err);
            else {
                var upduser = getuser;
                upduser.name = usr.name;
                upduser.email = usr.email;
                upduser.gender = usr.gender;
                upduser.available = usr.available;
                upduser.save();
            }
        });
    }
...
passport.use(new LocalStrategy(function (username, password, done) {
  ...
  app.post('/api/addDiscount', function (req, res) {
      ...
      if (req.isAuthenticated() && req.session.passport.user[0].adminStatus) {
             lunchedin.addDiscount(req.body);
  ...
  app.get('/api/restaurantdiscounts', function (req, res) {
      if (req.isAuthenticated()) {   //!! TODO: find if this is safe? I think there's a loophole - if req can be tampered around with
          ...
          RestaurantDiscount.find(function (err, restaurants) {
  ...
  app.post('/api/deleteUser', function (req, res) {
      if (req.isAuthenticated() && req.session.passport.user[0].adminStatus) {
          User.findOne({
              ...
              if (user != null) {
                  var upduser = user;
                  upduser.isDisable = true;
                  upduser.save();
              }

              res.send("Admin Request Approved: Updated user");
  ...
  app.post('/api/updateStatus', function (req, res) {
      ...
      if (req.isAuthenticated() && req.session.passport.user[0].adminStatus) {
          ...
          User.findOne({
              ...
              if (user != null) {
                  var upduser = user;
                  upduser.isPause = req.body.isPause;
                  upduser.save();
              }
...
//
  ...
  app.post('/api/addCompany', function (req, res) {
      ...
      if (req.isAuthenticated() && req.session.passport.user[0].adminStatus) {
          ...
          Company.find({
              ...
              if (err) console.log("Error while adding company", err);

              // user is always an array - remember this!
              if (company.length == 0)
                  lunchedin.addCompany(req.body);

              res.send("Admin Request Approved: Added new company");
  ...
  app.post('/api/updaterating', function (req, res) {
      console.log(req.body);
      // only Admins can add new users
      if (req.isAuthenticated() && req.session.passport.user[0].adminStatus) {

          lunchedin.updateRating(req.body);
          res.send("Admin Request Approved: Updated company");
  ...
  app.post('/api/updateCompany', function (req, res) {
      ...
      if (req.isAuthenticated() && req.session.passport.user[0].adminStatus) {
          ...
          Company.findOne({
              ...
              // user is always an array - remember this!
              if (company!=null)
                  lunchedin.updateCompany(req.body);
  ...
  app.post('/api/deleteCompany', function (req, res) {
      ...
      if (req.isAuthenticated() && req.session.passport.user[0].adminStatus) {
          ...
          Company.findOne({
              ...
              if (err) console.log("Error while deleting company", err);

              // user is always an array - remember this!
              if (company != null)
              {
                  console.log(company);
                  var updCompany = company;
                  updCompany.isdisable = true;
                  updCompany.save();
              }

              res.send("Admin Request Approved: Deleted company");
  ...
  // Sends all the companies in the database 
  app.get('/api/companies', function (req, res) {
      ...
      if (req.isAuthenticated()) {   //!! TODO: find if this is safe? I think there's a loophole - if req can be tampered around with
          ...
          // get mongoose to extract all users in the database
          Company.find({'isdisable':false},function (err, companies) {
              console.log(err);
...
/* User KPI Calculation APIs */
  app.post('/api/newpeoplemet', function (req, res) {
      console.log('-------newpeoplemet----------');
      ...
      Match.find(criteria, function (err, glmatches) {
          console.log(glmatches);
          res.json(glmatches);
      });
  });

// User edge length
  app.post('/api/getuseredge', function (req, res) {
      ...
      Match.find(criteria, function (err, glmatches) {
          User.findOne({
              ...
              res.json({ 'user': getuser, 'matches': glmatches });
          });
      });
  });

// avg path lenght of network in company
  app.post('/api/getpathlength', function (req, res) {
      if (req.isAuthenticated()) {
          ...
          if (cid == 0) {
              cid = req.session.passport.user[0].companyid;
              console.log(cid);
          }
              ...
              }, function (err, getusers) {
                  Company.findOne({ "_id": ObjectId(cid) }, function (err, getcompany) {
                      console.log(getusers);
                      var userids = [];
                      for (var i = 0; i < getusers.length; i++)
                          userids.push(ObjectId(getusers[i]._id));
  ...
  //app.post('/api/getrestaurantkpi', function (req, res) {
  //    console.log('-------getuseredge----------');
  //    console.log(new Date(req.body.startdate));
  //    Restaurant.findOne({ "_id": ObjectId(req.query.id) }, function (err, restaurants) {
  //        RestaurantDiscount.find({ "restaurantid": ObjectId(req.query.id) }, function (err, restaurantdiscounts) {
  //            if (err)
  //                res.send(err);
  //            else
  //                res.json({ "restaurants": restaurants, "discounts": restaurantdiscounts });
  //        });
  //    });
  //});
